<main>
   <div class="vd jd tto vv oq ar ri">
      ...
      <div class="_y js jn id">
         <!-- Left: Title -->
         <div class="rw _a">
            <h1 class="gg zj text-slate-800 font-bold">Solicitudes realizadas✨</h1>
         ...
         <div class="rw _a">
            <ul class="flex flex-wrap -m-1">
               <li class="m-1">
                  ...
                  <button class="inline-flex items-center justify-center text-sm gk yr rounded-full me mt border border-slate-200 hover--border-slate-300 bw bg-white text-slate-500 wu wc">Todos <span class="rq yu">
                     <?php
                     $clients = $this->db->query("SELECT COUNT(*) as Total FROM clients where completion_status = 'Activo'")->row_array();
                      echo $clients['Total'];
                     ?>
                  </span></button>
                  </a>
               ...
               <li class="m-1">
                   <a href="<?php echo base_url();?>admin/request_made">
                  <button class="inline-flex items-center justify-center text-sm gk yr rounded-full me mt border he bw hb yo wu wc">Solicitudes realizadas <span class="rq text-indigo-200">
                       <?php
         ...
         <div class="sq fa _j fm ji fy">
            ...
            <div class="table-items-action hidden">
               <div class="flex items-center">
                  ...
                  <button id="btndeleterequest" class="btn bg-white border-slate-200 hover--border-slate-300 yl xb">Eliminar</button>
               </div>
            </div>
            <!-- Dropdown -->
            ...
            <!-- Filter button -->
             <div class="td inline-flex">
               ...
               <button class="btn bg-white border-slate-200 hover--border-slate-300 text-slate-500 hover--text-slate-600">
                 ...
                 <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-left" width="22" height="22" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="lz">
               <table id="myTable" class="av oq">
                  ...
                  <tbody class="text-sm lg lb">
                     ...
                     <?php foreach($reque_made as $clts):?>
                     <?php if($clts['priority']!='' && $clts['completion_status']):?>
                     <tr>
                        <td class="v_ wk xe vm co ut">
                           <div class="flex items-center"> <label class="inline-flex"> <span class="tc">Select</span> <input class="table-item to" id="check2" value="<?php echo $clts['request_id']?>" type="checkbox" @click="uncheckParent"> </label> </div>
                        </td>
   ...
   <!-- Start -->
   <div x-data="{ modalOpen: false }">
      <button class="yl xb rounded-full" id="" onclick="eliminarRequest('<?php echo $clts['request_id']?>')" >
      ...
      <div id="feedback-modal" class="th tm nm lq flex items-center rf justify-center fe vd jd" role="dialog" aria-modal="true" x-show="modalOpen" x-transition:enter="wr wc wf" x-transition:enter-start="opacity-0 a_" x-transition:enter-end="bv ax" x-transition:leave="wr wc wf" x-transition:leave-start="bv ax" x-transition:leave-end="opacity-0 a_" style="display: none;">
         <div class="bg-white rounded by lj aa oq ok" @click.outside="modalOpen = false" @keydown.escape.window="modalOpen = false">
            ...
            <div class="mn vm cx border-slate-200">
               <div class="flex fg items-center">
                  <div class="g_ text-slate-800">Editar solicitud</div>
                  <button class="yu xm" @click="modalOpen = false">
                     <div class="tc">Close</div>
                     <svg class="ue on db">
                        <path d="M7.95 6.536l4.242-4.243a1 1 0 111.415 1.414L9.364 7.95l4.243 4.242a1 1 0 11-1.415 1.415L7.95 9.364l-4.243 4.243a1 1 0 01-1.414-1.415L6.536 7.95 2.293 3.707a1 1 0 011.414-1.414L7.95 6.536z"></path>
            ...
            <div class="gk text-slate-800 it"></div>
            <?php
               $url = "admin/update_request/".$clts['client_id'];
            ?>
            <form action="<?php echo base_url($url)?>" method="POST">
               <div class="ls">
                  <div class="_y jn ln ju jl ig">
                  ...
                  <div class="_xx">
                     <label class="block text-sm gk rx" for="start_date">Fecha de término <span class="yl">*</span></label>
                     <input id="end_date" name="end_date" class="tn oq v_ mt" type="date">
                  </div>     
               </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
     $("#btndeleterequest").on("click", function(){
         var bool=confirm("¿Estas seguro de eliminar el historial de solicitudes?");
            if(bool){
         var checks = document.querySelectorAll('#check2');
         checks.forEach((e)=>{
             if(e.checked==true){
                 ...
                 $.ajax({
                 data: parametros,
                 url: '<?php echo base_url('Admin/deleteAllrequest_made');?>',
                 type: 'POST',
...
function eliminarRequest(id) {
    Swal.fire({
        title: "¿Estás seguro?",
        text: "Se eliminará todo el registro de la solicitud",
        icon: "question",
        iconColor: "red",
        ...
        if (result.value) {
            ...
            location.href = "<?php echo base_url('/admin/deleteRequest_made')?>/" + id;
        }
    });
}
