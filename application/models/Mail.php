...
class Mail extends School 
{
    ...
    */
    ...
    function submitPassword($email, $auth_key)
    {
        $name   = $this->db->get_where('settings', array('type' => 'name'))->row()->description;
        
        $email_sub  = 'Reestablecimiento de cuenta';
        $email_msg  .= 'Reestablecimiento de acceso'."<br>";
        $email_msg  .= 'Hemos recibido una solicitud para cambiar tu contraseña el <b>'.date('d').', de '.date('M').' a las '.date('H:i').'</b>, para continuar y cambiar tu contraseña haz click en el botón de abajo, el enlace expira en 30 minutos.<br>
                                    <p>Si no has sido tú, ignora este mensaje, mantendremos segura tu cuenta.</p><hr>
                                    <a href="'.base_url().'login/new_request?auth='.$auth_key.'" style="padding: 8px 20px; background-color: #703296; border-radius: 5px; color: #fff; font-weight: bolder; font-size: 16px; display: inline-block; margin: 20px 0px; margin-right: 20px; text-decoration: none; -webkit-box-shadow: 0px 2px 14px rgba(0, 68, 233, 0.40);
	...
	{
	    if($this->db->get_where('settings', array('type' => 'smtp_user'))->row()->description != '')
	    {
	        $config = Array(
